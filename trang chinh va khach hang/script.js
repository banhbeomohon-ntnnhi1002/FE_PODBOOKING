...
// gọi API cho thông báo
let Notifications = [];

async function fetchNotifications() {
  try {
    const userEmail = localStorage.getItem('userEmail');
    const res = await fetch(`http://127.0.0.1:6868/notifications?user=${encodeURIComponent(userEmail)}`);
...
function updateRecentBookings() {
  ...
  mockUserBookings.forEach((booking, index) => {
    ...
    deleteButton.addEventListener('click', () => { //xóa lịch sử đặt chỗ
      const bookingId = booking.id; 
      fetch(`http://127.0.0.1:6868/bookings/${bookingId}`, { method: 'DELETE' })
        .then(res => {
          if (!res.ok) throw new Error('API lỗi');
          showNotification('Xóa lịch sử đặt chỗ thành công!');
          fetchUserBookings(); 
        })
        .catch(() => showNotification('Lỗi xóa đặt chỗ', 'error'));
    });
...
function updateNotificationList() {
  ...
  fetch(`http://127.0.0.1:6868/notifications/${notificationId}`, { method: 'DELETE' })
    .then(res => {
      if (!res.ok) throw new Error('API lỗi');
      showNotification('Xóa thông báo thành công!');
      fetchNotifications();
    })
    .catch(() => showNotification('Lỗi xóa thông báo', 'error'));
});
      li.appendChild(deleteButton);
      notificationListItems.appendChild(li);
    });
...
// gọi API cho khung giờ trống
const mockAvailability = {};

async function fetchAvailability(date) {
  try {
    const res = await fetch(`http://127.0.0.1:6868/availability?date=${encodeURIComponent(date)}`);
    if (!res.ok) throw new Error('API lỗi');
    const data = await res.json();
    mockAvailability[date] = data.slots;
    updateAvailableSlots(date, bookingAvailableSlots, bookingAvailableSlotsList, bookingTime);
  } catch (err) {
    showNotification('Lỗi tải khung giờ trống', 'error');
  }
}

// Gọi khi chọn ngày
bookingDate.addEventListener('change', () => {
  fetchAvailability(bookingDate.value);
});

// gọi API cho lịch làm việc
let mockSchedule = [];

async function fetchSchedule() {
  try {
    const userEmail = localStorage.getItem('userEmail');
    const res = await fetch(`http://127.0.0.1:6868/schedule?user=${encodeURIComponent(userEmail)}`);
    if (!res.ok) throw new Error('API lỗi');
    const data = await res.json();
    mockSchedule = data;
    updateScheduleList();
...
//gọi API cho gói dịch vụ
let mockPackages = [];

async function fetchPackages() {
  try {
    const userEmail = localStorage.getItem('userEmail');
    const res = await fetch(`http://127.0.0.1:6868/packages?user=${encodeURIComponent(userEmail)}`);
    if (!res.ok) throw new Error('API lỗi');
    const data = await res.json();
    mockPackages = data;
    updatePackageList();
    ...
    showNotification('Lỗi tải gói dịch vụ', 'error');
  }
}
// gọi API cho lịch sử đặt chỗ
let mockUserBookings = [];

async function fetchUserBookings() {
  try {
    const userEmail = localStorage.getItem('userEmail');
    const res = await fetch(`http://127.0.0.1:6868/bookings?user=${encodeURIComponent(userEmail)}`);
    if (!res.ok) throw new Error('API lỗi');
    const data = await res.json();
    mockUserBookings = data;
    updateRecentBookings();
    updatePaymentItems();
  } catch (err) {
    showNotification('Lỗi tải lịch sử đặt chỗ', 'error');
  }
}
...
function updateScheduleList() {
  ...
  mockSchedule.forEach((item, index) => {
    ...
    const deleteButton = document.createElement('button');
    deleteButton.textContent = 'Xóa';
    deleteButton.addEventListener('click', () => { // xóa lịch làm việc
      const scheduleId = item.id; 
      fetch(`http://127.0.0.1:6868/schedule/${scheduleId}`, { method: 'DELETE' })
        .then(res => {
          if (!res.ok) throw new Error('API lỗi');
          showNotification('Xóa lịch làm việc thành công!');
          fetchSchedule();
        })
        .catch(() => showNotification('Lỗi xóa lịch làm việc', 'error'));
    });
    li.appendChild(deleteButton);
...
function updatePackageList() {
  ...
  mockPackages.forEach((item, index) => {
    ...
    deleteButton.addEventListener('click', () => { // hủy gói dịch vụ
      const packageId = item.id; 
      fetch(`http://127.0.0.1:6868/packages/${packageId}`, { method: 'DELETE' })
        .then(res => {
          if (!res.ok) throw new Error('API lỗi');
          showNotification('Hủy gói dịch vụ thành công!');
          fetchPackages();
        })
        .catch(() => showNotification('Lỗi hủy gói dịch vụ', 'error'));
    });
    li.appendChild(deleteButton);
...
loginForm.addEventListener('submit', (e) => {
  ...
  //API đăng nhập
  if (valid) {
    fetch('http://127.0.0.1:6868/login', {
      method: 'POST',
    ...
    .catch(() => showNotification('Lỗi đăng nhập', 'error'));
  }
});
...
signupForm.addEventListener('submit', (e) => {
  ...
  // số điện thoại
  const numberphone = document.getElementById('signup-numberphone').value;
  const numberphoneError = document.getElementById('signup-numberphone-error');
  const phoneRegex = /^(0|\+84)[0-9]{9}$/;
  if (!phoneRegex.test(numberphone)) {
    numberphoneError.textContent = 'Vui lòng nhập số điện thoại hợp lệ (10 số, bắt đầu bằng 0 hoặc +84)';
  ...
  // API đăng kí
  if (valid) {
    fetch('http://127.0.0.1:6868/signup', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name, email, numberphone, password })
    })
    .then(res => {
      if (!res.ok) throw new Error('API lỗi');
      return res.json();
    ...
    .catch(() => showNotification('Lỗi đăng ký', 'error'));
  }
});
...
// API đặt chỗ
  if (valid) {
    const userEmail = localStorage.getItem('userEmail');
    fetch('http://127.0.0.1:6868/bookings', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user: userEmail, date, podType })
    })
    .then(res => {
      if (!res.ok) throw new Error('API lỗi');
      return res.json();
    ...
    .catch(() => showNotification('Lỗi đặt chỗ', 'error'));
  }
});
...
scheduleForm.addEventListener('submit', (e) => {
  ...
  //API thêm lịch làm việc
  if (valid) {
    const userEmail = localStorage.getItem('userEmail');
    fetch('http://127.0.0.1:6868/schedule', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user: userEmail, date, time, task })
    })
    .then(res => {
      if (!res.ok) throw new Error('API lỗi');
      return res.json();
    ...
    .catch(() => showNotification('Lỗi thêm lịch làm việc', 'error'));
  }
});
...
packageForm.addEventListener('submit', (e) => {
  ...
  // API thêm gói dịch vụ
  if (valid) {
    const price = duration * (type === 'hour' ? 30000 : type === 'day' ? 200000 : 600000);
    fetch('http://127.0.0.1:6868/packages', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user: userEmail, type, duration, price })
    })
    .then(res => {
      if (!res.ok) throw new Error('API lỗi');
      return res.json();
    ...
    .catch(() => showNotification('Lỗi thêm gói dịch vụ', 'error'));
  }
});
...
paymentForm.addEventListener('submit', (e) => {
  ...
  if (valid) { // API thanh toán
    let apiUrl = '';
    let payload = {};
    if (item.startsWith('package-')) {
      const pkgIndex = parseInt(item.split('-')[1]);
      const packageId = mockPackages[pkgIndex].id;
      apiUrl = `http://127.0.0.1:6868/packages/${packageId}/pay`;
      payload = { paid: true, amount, method };
    } else {
      const bookingIndex = parseInt(item);
      const bookingId = mockUserBookings[bookingIndex].id;
      apiUrl = `http://127.0.0.1:6868/bookings/${bookingId}/pay`;
      payload = { paid: true, amount, method };
    }
    fetch(apiUrl, {
      method: 'POST',
    ...
    .catch(() => showNotification('Lỗi thanh toán', 'error'));
  }
});
...
customizeForm.addEventListener('submit', (e) => {
  ...
  if (valid) {
    const userEmail = localStorage.getItem('userEmail');
    fetch('http://127.0.0.1:6868/customize', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user: userEmail, color, font })
    })
    .then(res => {
      if (!res.ok) throw new Error('API lỗi');
      // Áp dụng giao diện sau khi lưu thành công
    ...
    .catch(() => showNotification('Lỗi lưu tùy chỉnh', 'error'));
  }
});